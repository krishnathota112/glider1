...
% DIR_INPUT_NC_FILES = 'C:\Users\jprannou\_DATA\GLIDER\slocum\';
...
if (isempty(dataToProcessDir))
   ...
   for dirNum = 1:length(dirInfo)
      if ~(strcmp(dirInfo(dirNum).name, '.') || strcmp(dirInfo(dirNum).name, '..'))
         ...
         deployName = dirName;
         nc_ego_2_csv_file([DIR_INPUT_NC_FILES '/' dirName], deployName);
      end
   end
else
   % convert the data of this deployment
   nc_ego_2_csv_file(dataToProcessDir, deployName);
end
...
%
% SYNTAX :
%  nc_ego_2_csv_file(a_dirName, a_deployName)
%
% INPUT PARAMETERS :
...
%   04/11/2016 - RNU - creation
% ------------------------------------------------------------------------------
function nc_ego_2_csv_file(a_dirName, a_deployName)

% global default values
global g_decGl_dateDef;


ncFiles = dir([a_dirName '/' a_deployName '*.nc']);
for idF = 1:length(ncFiles)
   ...
   phase2 = gl_get_data_from_name('PHASE2', timeData);
   phaseNumber2 = gl_get_data_from_name('PHASE_NUMBER2', timeData);
   ...
   if (isempty(phase2))
      ...
      if (~isempty(latitude))
         fprintf(fidOut, '#; Date; Qc; Lon; Lat; Qc; Pos method; Phase; Phase #; Phase2; Phase2 #');
      else
         fprintf(fidOut, '#; Date; Qc; Phase; Phase #; Phase2; Phase2 #');
      end
   end
   ...
   if (isempty(phase2))
      ...
      for idJ = 1:length(juld)
         if (~isempty(latitude))
            fprintf(fidOut, '%d; %s; %d; %.3f; %.3f; %d; %d; %d; %d; %d; %d', ...
               idJ, gl_julian_2_gregorian(juld(idJ)), juldQc(idJ), longitude(idJ), latitude(idJ), positionQc(idJ), posMethod(idJ), phase(idJ), phaseNumber(idJ), phase2(idJ), phaseNumber2(idJ));
         else
            fprintf(fidOut, '%d; %s; %d; %d; %d; %d; %d', ...
               idJ, gl_julian_2_gregorian(juld(idJ)), juldQc(idJ), phase(idJ), phaseNumber(idJ), phase2(idJ), phaseNumber2(idJ));
         end
         for idM = 1:length(paramList)
            fprintf(fidOut, '; %g; %d', paramData(idJ, idM), paramDataQc(idJ, idM));
         end
   ...
   wcQc = gl_get_data_from_name('WATERCURRENTS_QC', currentData);
   wcErr = gl_get_data_from_name('WATERCURRENTS_ERROR', currentData);
   ...
   if (~isempty(wcTime))
      ...
      if (~isempty(wcJuld))
         if (isempty(wcQc))
            ...
            fprintf(fidOut, 'CURRENT DATA\n');
            fprintf(fidOut, '********\n');
            fprintf(fidOut, '#; Date; Lon; Lat; Depth; U; V; QC; ERROR\n');
            fprintf(fidOut, '-------\n');

            for id = 1:length(wcJuld)
               fprintf(fidOut, '%d; %s; %.3f; %.3f; %.1f; %.1f; %.1f; %d; %.1f\n', ...
                  id, gl_julian_2_gregorian(wcJuld(id)), wcLon(id), wcLat(id), wcDepth(id), wcU(id), wcV(id), wcQc(id), wcErr(id));
            end
         end
      end
   end

   surfWcTime = gl_get_data_from_name('SURF_WATERCURRENTS_TIME', currentData);
   surfWcLat = gl_get_data_from_name('SURF_WATERCURRENTS_LATITUDE', currentData);
   surfWcLon = gl_get_data_from_name('SURF_WATERCURRENTS_LONGITUDE', currentData);
   surfWcU = gl_get_data_from_name('SURF_WATERCURRENTS_U', currentData);
   surfWcV = gl_get_data_from_name('SURF_WATERCURRENTS_V', currentData);
   surfWcQc = gl_get_data_from_name('SURF_WATERCURRENTS_QC', currentData);
   surfWcErr = gl_get_data_from_name('SURF_WATERCURRENTS_ERROR', currentData);

   if (~isempty(surfWcTime))

      % compute Julian 1950 dates from EPOCH 1970 dates
      paramTime = gl_get_netcdf_param_attributes('TIME');
      paramJuld = gl_get_netcdf_param_attributes('JULD');
      surfWcJuld = ones(size(surfWcTime))*paramJuld.fillValue;
      idNoDef = find(surfWcTime ~= paramTime.fillValue);
      epoch_offset = datenum(1970, 1, 1) - datenum(1950, 1, 1);
      surfWcJuld(idNoDef) = surfWcTime(idNoDef)/86400 + epoch_offset;

      fprintf(fidOut, '********\n');
      fprintf(fidOut, 'SURFACE CURRENT DATA\n');
      fprintf(fidOut, '********\n');
      fprintf(fidOut, '#; Date; Lon; Lat; U; V; QC; ERROR\n');
      fprintf(fidOut, '-------\n');

      for id = 1:length(surfWcJuld)
         fprintf(fidOut, '%d; %s; %.3f; %.3f; %.1f; %.1f; %d; %.1f\n', ...
            id, gl_julian_2_gregorian(surfWcJuld(id)), surfWcLon(id), surfWcLat(id), surfWcU(id), surfWcV(id), surfWcQc(id), surfWcErr(id));
      end
   end
