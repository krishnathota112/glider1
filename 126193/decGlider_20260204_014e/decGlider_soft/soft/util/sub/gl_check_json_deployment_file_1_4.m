...
%
% SYNTAX :
%  [o_outputData] = gl_check_json_deployment_file_1_4(a_deployJsonFilePathName)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_outputData] = gl_check_json_deployment_file_1_4(a_deployJsonFilePathName)
...
fprintf('INFO: checking deployment json file: %s\n', a_deployJsonFilePathName);
...
for idGc = 1:size(checkMaxSizeFields, 1)
   ...
   if (iscellstr(attValue))
      ...
      for idV = 1:length(attValueList)
         attValue = attValueList{idV};
         if ~(strncmp(attValue, '<nc.global_att.', length('<nc.global_att.')) || ...
               strncmp(attValue, '<nc.data.', length('<nc.data.')))
...
for idFile = 1:length(sensorFiles)
   ...
   % fprintf('INFO: checking sensor json file: %s\n', sensorFilePathName);
