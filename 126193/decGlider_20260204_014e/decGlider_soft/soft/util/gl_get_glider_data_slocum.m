...
%
% SYNTAX :
%   gl_get_glider_data_slocum or
%   gl_get_glider_data_slocum('data', 'crate_mooset00_38')
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function gl_get_glider_data_slocum(varargin)
...
logFile = [DIR_LOG_FILE '/' 'gl_get_glider_data_slocum_' deploymentDir '_' datestr(now, 'yyyymmddTHHMMSS') '.log'];
diary(logFile);
tic;

% create the CSV output file
outputFileName = [DIR_CSV_FILE '/' 'gl_get_glider_data_slocum_' deploymentDir '_' datestr(now, 'yyyymmddTHHMMSS') '.csv'];
fidOut = fopen(outputFileName, 'wt');
if (fidOut == -1)
   return
end
header = 'DEPLOYMENT;GLIDER PARAMETER;AVAILABLE DATA;JSON FILE;EGO PARAMETER;EGO SENSOR;EGO SENSOR_MAKER;EGO SENSOR_MODEL;EGO SENSOR_SERIAL_NO';
fprintf(fidOut, '%s\n', header);
...
if (~found)
   [availableParam, availableData] = gl_get_available_data_slocum(a_deploymentTopDirName, a_deploymentDirName);
   aDataStruct = '';
...
for idP = 1:length(availableParam)
   ...
   if (~isempty(idF{:}))
      ...
      if (isempty(idF))
         ...
         fprintf(a_csvFid, '%s;%s;%d;%s;%s;%s;%s;%s;%s\n', ...
            a_deploymentDirName, ...
            availableParam{idP}, availableData(idP), ...
...
for idP = 1:length(gliderNames)
   if (isempty(gliderNames{idP}))
      fprintf(a_csvFid, '%s;none;-1;%s;%s;%s;%s;%s;%s\n', ...
         a_deploymentDirName, ...
         jsonFile{idP}, paramNames{idP}, sensorNames{idP}, ...
         sensorMaker{idP}, sensorModel{idP}, sensorSerialNo{idP} ...
         );
   end
end
...
%
% SYNTAX :
% [o_sensorName, o_paramName, o_gliderName, ...
...
%   10/24/2023 - RNU - creation
% ------------------------------------------------------------------------------
function [o_sensorName, o_paramName, o_gliderName, ...
   o_sensorMaker, o_sensorModel, o_sensorSerialNo, o_jsonFile] = get_link_to_data( ...
   a_deploymentTopDirName, a_deploymentDirName)
