...
%                             specified).
%   12/12/2023 - RNU - V 1.10: Existing tests applied on BBP470, BBP470_2,
%                              BBP700_2 and DOXY3 parameters.
%   02/04/2025 - RNU - V 1.12: New general rule: "if PRES_QC=4 then <PARAM>_QC=4
                  ...
                  if (~isempty(profPres))
                     ...
                     if (length(profPres) > 1)
                        ...
                        if (~isempty(idToFlag))
                           ...
                           presQC4TestNumList = [presQC4TestNumList, 8];
                        end
                     end
                  end
               end
               ...
               for idProf = 1:length(juld)
                  ...
                  if ((length(idNoDef) > 1) && (length(uProfData) == 1))
                     ...
                     if (strncmp(paramName, 'PRES', length('PRES')))
                        presQC4ProfIdList = [presQC4ProfIdList, idProf];
                        presQC4TestNumList = [presQC4TestNumList, 13];
                     end
                  end
               end
            end
   ...
   for idD = 1:2
      ...
      if (~isempty(idPres))
         ...
         if (~isempty(presData))
            ...
            profIdList = unique(presQC4ProfIdList);
            for idProf = profIdList
               
               profPres = presData(idProf, :);
               profPresQc = presDataQc(idProf, :);
               if (~isempty(profPres))
                  if (any(profPresQc == g_decGl_qcStrBad))
                     ...
                     for idP = 1:length(g_decGl_paramWithRtqcTest)
                        ...
                        if (~isempty(idParam))
                           ...
                           if (~isempty(paramData))
                              ...
                              profParamQc = presDataQc(idProf, :);

                              if (~isempty(profParam))

                                 % check that the RTQC tests have been used for this
                                 % parameter
                                 uProfParamQc = unique(profParamQc);
                                 uProfParamQc(uProfParamQc == g_decGl_qcStrDef) = [];
                                 uProfParamQc(uProfParamQc == g_decGl_qcStrInterpolated) = [];
                                 uProfParamQc(uProfParamQc == g_decGl_qcStrMissing) = [];

                                 if (any(uProfParamQc ~= g_decGl_qcStrNoQc))

                                    % apply the test
                                    idNoDef = find((profPres ~= presDataFillValue) & ...
                                       (profParam ~= paramFillValue));
                                    idToFlag = find((presDataQc(idNoDef) == g_decGl_qcStrBad));
                                    if (~isempty(idToFlag))
                                       paramDataQc(idProf, idNoDef(idToFlag)) = gl_set_qc_str(paramDataQc(idProf, idNoDef(idToFlag)), g_decGl_qcStrBad);
                                       eval([ncParamXDataQcList{idParam} ' = paramDataQc;']);

                                       idF = find(presQC4ProfIdList == idProf, 1, 'first');
                                       if (~isempty(idF))
                                          testFailedList(presQC4TestNumList(idF), idProf) = 1;
                                       end
                                    end
                                 end
                              end
...
DIR_TMP_FILE = [monoProfInputPath filesep 'tmp' filesep];
...
if (ok == 1)
   ...
   movefile(tmpNcMonoProfOutputPathFileName, [monoProfOutputPath filesep fileName fileExtension]);
end
