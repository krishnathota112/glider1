...
while (~strncmp('%data:', gg, 5))
   ...
   elseif (strncmp('%basestation_version:', gggg{1}, 21))
      ...
      if (~isempty(space_idx))
         tvar = gg(2:space_idx(1)-2);
         if (length(space_idx) == 1)
            o_sgStructure.bpo.(tvar) = str2double(gg(space_idx(1)+1:end));
         else
            o_sgStructure.bpo.(tvar) = {gg(space_idx(1)+1:end)}; % the entires with multiple values include strings too, not parsing fully in this version of code
         end
      end
   end
   gg = fgetl(fIdIn);
...
% duplicate elapsed_t_bin in timeGliderVarName
o_sgStructure.bpo.(timeGliderVarName) = o_sgStructure.bpo.elapsed_t_bin;
