...
if ~(isfield(deployJsonData, 'EGO_format_version') && ...
      ...
      (str2double(deployJsonData.EGO_format_version) == g_decGl_egoFormatVersion))
   fprintf('ERROR: expected ''%.1f'' EGO format version in json file (%s)\n', ...
      g_decGl_egoFormatVersion, a_deployJsonFilePathName);
   return
end
if (isfield(deployJsonData, 'EGO_format_version'))
...
if (isempty(glSensorFile))
   fprintf('WARNING: no ''glider_sensor'' information in json file (%s)\n', ...
      a_deployJsonFilePathName);
   return
end
error = 0;
...
checkInfoList = {
   ...
   'PARAMETER', 0, '3'; ...
   'PARAMETER_SENSOR', 0, '25'; ...
   'PARAMETER_DATA_MODE', 0, '19' ...
   };
...
% add path to sensor file names
[jsonDir, ~, ~] = fileparts(a_deployJsonFilePathName);
sensorFiles = deployJsonData.glider_sensor;
for idFile = 1:length(sensorFiles)
   sensorFiles(idFile).sensor_file_name = [jsonDir filesep sensorFiles(idFile).sensor_file_name];
end

% add parameter TECH information from reference JSON files
[jsonRefDir, ~, ~] = fileparts(g_decGl_egoFormatJsonFile);
jsonRefDir = [jsonRefDir filesep];
if (strcmp(g_decGl_gliderType, 'slocum'))
   techSensorRefFile = [jsonRefDir sprintf('slocum_sensor_TECH_%.1f.json', g_decGl_egoFormatVersion)];
elseif (strcmp(g_decGl_gliderType, 'seaglider'))
   techSensorRefFile = [jsonRefDir sprintf('seaglider_sensor_TECH_%.1f.json', g_decGl_egoFormatVersion)];
elseif (strcmp(g_decGl_gliderType, 'seaexplorer'))
   techSensorRefFile = [jsonRefDir sprintf('seaexplorer_sensor_TECH_%.1f.json', g_decGl_egoFormatVersion)];
end
if (exist(techSensorRefFile, 'file') == 2)
   fprintf('INFO: adding TECH sensor file to deployment information: %s\n', techSensorRefFile);
   if (~isempty(sensorFiles))
      sensorFiles(end+1) = sensorFiles(end);
      sensorFiles(end).sensor_file_name = techSensorRefFile;
   else
      sensorFiles.sensor_file_name = techSensorRefFile;
   end
else
   fprintf('ERROR: expected TECH sensor file not found: %s\n', techSensorRefFile);
end

% check sensor files
error = 0;
for idFile = 1:length(sensorFiles)
   sensorFilePathName = sensorFiles(idFile).sensor_file_name;
   if ~(exist(sensorFilePathName, 'file') == 2)
      ...
      try
         ...
         if (strcmp(sensorData.SENSOR, 'GLIDER_TECH'))
            for idP = 2:length(dimParamList)
               if (strcmp(dimParamList{idP}, 'PARAMETER_SENSOR'))
                  sensorData.(dimParamList{idP}) = repmat({'GLIDER_TECH'}, 1, length(sensorData.PARAMETER));
               elseif (strcmp(dimParamList{idP}, 'PARAMETER_DATA_MODE'))
                  sensorData.(dimParamList{idP}) = repmat({'R'}, 1, length(sensorData.PARAMETER));
               else
                  sensorData.(dimParamList{idP}) = repmat({' '}, 1, length(sensorData.PARAMETER));
               end
            end
         end
      catch MException
      ...
      if ~(isfield(sensorData, 'EGO_format_version') && ...
         ...
         fprintf('ERROR: expected ''%.1f'' EGO format version in json file (%s)\n', ...
            g_decGl_egoFormatVersion, sensorFilePathName);
         return
      end
      if (isfield(sensorData, 'EGO_format_version'))
      ...
      if (isfield(sensorData, 'PARAMETER_DATA_MODE') && ~isempty(sensorData.PARAMETER_DATA_MODE))
