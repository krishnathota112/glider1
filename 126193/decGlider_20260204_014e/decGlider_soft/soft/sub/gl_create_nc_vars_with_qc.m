...
for idParamStruct = 1:length(paramListStruct)
   ...
   for idLoop = 1:5
      ...
      if ((idLoop == 1) || (idLoop == 3)) % <PARAM> & <PARAM>_ADJUSTED
         ...
         elseif (iscell(dimNames))
            ...
            if (g_decGl_netCDFCompressionFlag)
               if (g_decGl_netCDFCompressAll)
                  netcdf.defVarDeflate(o_ncFileId, varId, true, true, g_decGl_netCDFDeflateLevel);
               end
            end
         elseif (size(dimNames, 1) > 1)
            ...
            if (g_decGl_netCDFCompressionFlag)
               if (g_decGl_netCDFCompressAll)
                  ...
                  if (strcmp(varStruct.dim, 'TIME') || strcmp(varStruct.dim, 'TIME_GPS'))
                     netcdf.defVarDeflate(o_ncFileId, varId, true, true, g_decGl_netCDFDeflateLevel);
                  end
               end
            end
      ...
      elseif ((idLoop == 2) || (idLoop == 4)) % <PARAM>_QC & <PARAM>_ADJUSTED_QC
         ...
         if ((length(varName) <= 3) || ((length(varName) > 3) && ~strcmp(varName(end-2:end), '_QC')))
            ...
            if (gl_var_is_present(o_ncFileId, varNameQc) == 0)
               ...
               if (g_decGl_netCDFCompressionFlag)
                  if (g_decGl_netCDFCompressAll)
                     netcdf.defVarDeflate(o_ncFileId, varQcId, true, true, g_decGl_netCDFDeflateLevel);
                  else
                     if (strcmp(a_varQcDefStruct.dim, 'TIME') || strcmp(a_varQcDefStruct.dim, 'TIME_GPS'))
                        netcdf.defVarDeflate(o_ncFileId, varQcId, true, true, g_decGl_netCDFDeflateLevel);
                     end
                  end
               end
         ...
         if ((length(varName) <= 3) || ((length(varName) > 3) && ~strcmp(varName(end-2:end), '_QC')))
            ...
            if (g_decGl_netCDFCompressionFlag)
               if (g_decGl_netCDFCompressAll)
                  netcdf.defVarDeflate(o_ncFileId, varAdjErrId, true, true, g_decGl_netCDFDeflateLevel);
               else
                  if (strcmp(a_varAdjErrDefStruct.(fieldNamesAdjErr{3}), 'TIME') || strcmp(a_varAdjErrDefStruct.(fieldNamesAdjErr{3}), 'TIME_GPS'))
                     netcdf.defVarDeflate(o_ncFileId, varAdjErrId, true, true, g_decGl_netCDFDeflateLevel);
                  end
               end
            end
