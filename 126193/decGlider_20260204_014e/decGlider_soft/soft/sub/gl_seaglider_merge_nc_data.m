         ...
         if (~isempty(varTime))
            ...
            if (~isfield(inputDataStruct.(varDim).VAR, varName))
               ...
               if (isfield(a_sgStructure.VAR, varNameQc))
                  ...
                  % if (any(a_sgStructure.VAR.(varNameQc).DATA == '6'))
                  %    fprintf('ERROR: Imported netCDF var ''%s'' contains ''6'' values not managed by EGO format\n', varNameQc);
                  % end
               end
            else
            ...
            if (~isempty(varAdjName))
               if (~isfield(inputDataStruct.(varDim).VAR, varAdjName))
                  ...
                  if (isfield(a_sgStructure.VAR, varAdjNameQc))
                     ...
                     % if (any(a_sgStructure.VAR.(varAdjNameQc).DATA == '6'))
                     %    fprintf('ERROR: Imported netCDF var ''%s'' contains ''6'' values not managed by EGO format\n', varAdjNameQc);
                     % end
                  end
               else
...
if (~isempty(inputDataStruct))
   ...
   for idF1 = 1:length(fDimNames)
      ...
      for idF2 = 1:length(fVarNames)
         ...
         if (strcmp(varName, timeGliderVarName))
            continue
         end
   ...
   for idV = 1:length(g_decGl_egoVarName)
      if (~ismember(g_decGl_egoVarName{idV}, excludedEgoVarList) && ...
         ...
         if (isfield(mergedDataStruct, g_decGl_gliderVarName{idV}))
            ...
            % QC are also retrieved from input nc file
            if (isfield(mergedDataStruct, [g_decGl_gliderVarName{idV} '_qc']))
               dataAll = cat(1, dataAll, mergedDataStruct.([g_decGl_gliderVarName{idV} '_qc']));
            end
         end
      end
      % adjusted values are also retrieved from input nc file
      if (~ismember(g_decGl_egoVarName{idV}, excludedEgoVarList) && ...
            ~strcmp(g_decGl_egoVarName{idV}, 'TIME') && ...
            ~isempty(g_decGl_gliderAdjVarName{idV}))
         if (isfield(mergedDataStruct, g_decGl_gliderAdjVarName{idV}))
            dataAll = cat(1, dataAll, mergedDataStruct.(g_decGl_gliderAdjVarName{idV}));
            % QC are also retrieved from input nc file
            if (isfield(mergedDataStruct, [g_decGl_gliderAdjVarName{idV} '_qc']))
               dataAll = cat(1, dataAll, mergedDataStruct.([g_decGl_gliderAdjVarName{idV} '_qc']));
            end
         end
      end
   end
   ...
   if (~isempty(idDel))
      for idV = 1:length(g_decGl_egoVarName)
         if (~ismember(g_decGl_egoVarName{idV}, excludedEgoVarList) && ...
            ...
            % QC are also retrieved from input nc file
            if (isfield(mergedDataStruct, [g_decGl_gliderVarName{idV} '_qc']))
               mergedDataStruct.([g_decGl_gliderVarName{idV} '_qc'])(idDel) = [];
            end
         end
         % adjusted values are also retrieved from input nc file
         if (~ismember(g_decGl_egoVarName{idV}, excludedEgoVarList) && ...
               ~isempty(g_decGl_gliderAdjVarName{idV}))
            if (isfield(mergedDataStruct, g_decGl_gliderAdjVarName{idV}))
               mergedDataStruct.(g_decGl_gliderAdjVarName{idV})(idDel) = [];
            end
            % QC are also retrieved from input nc file
            if (isfield(mergedDataStruct, [g_decGl_gliderAdjVarName{idV} '_qc']))
               mergedDataStruct.([g_decGl_gliderAdjVarName{idV} '_qc'])(idDel) = [];
            end
         end
      end
   end
   ...
   if (~isempty(timeGliderVarName) && ~isempty(presGliderVarName))
      ...
      if (~isempty(time) && ~isempty(pres))
         ...
         if (~isempty(presQcGliderVarName))
            ...
            g_decGl_directEgoVarPathName{end+1} = 'vars_time.PRES_QC';
         end
      end
   end
else
