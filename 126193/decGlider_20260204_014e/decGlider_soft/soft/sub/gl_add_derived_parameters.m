...
for idP = 1:length(idSlopeOffset)
   ...
   if (~isempty(egoVarName) && ~isempty(gliderVarName))
      ...
      if (gl_is_field_recursive(o_rawData, gliderVarPathName))
         ...
         g_decGl_directEgoVarPathName{end+1} = [gl_get_path_to_data('') '.' egoVarName];
...
if (isempty(psal) && ...
   ...
   if (~isempty(psalEgoVarName) && isempty(psalGliderVarName))
      ...
      g_decGl_directEgoVarPathName{end+1} = [gl_get_path_to_data('') '.PSAL'];
...
if (~isempty(psal2EgoVarName) && isempty(psal2GliderVarName))
   ...
   if (~isempty(pres2) && ...
      ...
      g_decGl_directEgoVarPathName{end+1} = [gl_get_path_to_data('') '.PSAL2'];
      ...
      switch (caseValue)
         ...
         case {'201_201_301', '202_201_301'}
            ...
            if (~isempty(time) && ...
               ...
               g_decGl_directEgoVarPathName{end+1} = [gl_get_path_to_data('') '.' doxyEgoVarName];
...
for idP = 1:length(idBbp)
   ...
   if (~isempty(bbpEgoVarName) && isempty(bbpGliderVarName))
      ...
      % which BBP470 processing method is needed?
      if (isfield(g_decGl_calibInfo{g_decGl_calibInfoId}, 'ScaleFactBBP470') && ...
            isfield(g_decGl_calibInfo{g_decGl_calibInfoId}, 'DarkCountBBP470'))

         % BBP470 from BETA_BACKSCATTERING470
         betaBackscattering = get_values(o_rawData, 'BETA_BACKSCATTERING470');
      else
         % BBP470 from BETA_BACKSCATTERING470_SCALED
         betaBackscattering = get_values(o_rawData, 'BETA_BACKSCATTERING470_SCALED');
      end
      ...
      if (~isempty(time) && ...
         ...
         g_decGl_directEgoVarPathName{end+1} = [gl_get_path_to_data('') '.' bbpEgoVarName];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% compute BBP532 derived parameter

idBbp = find(strncmp(g_decGl_egoVarName, 'BBP532', length('BB532')));
for idP = 1:length(idBbp)
   bbpEgoVarName = g_decGl_egoVarName{idBbp(idP)};
   bbpGliderVarName = g_decGl_gliderVarName{idBbp(idP)};
   ...
   if (~isempty(bbpEgoVarName) && isempty(bbpGliderVarName))
      ...
      % which BBP532 processing method is needed?
      if (isfield(g_decGl_calibInfo{g_decGl_calibInfoId}, 'ScaleFactBBP532') && ...
            isfield(g_decGl_calibInfo{g_decGl_calibInfoId}, 'DarkCountBBP532'))

         % BBP532 from BETA_BACKSCATTERING532
         betaBackscattering = get_values(o_rawData, 'BETA_BACKSCATTERING532');
      else
         % BBP532 from BETA_BACKSCATTERING532_SCALED
         betaBackscattering = get_values(o_rawData, 'BETA_BACKSCATTERING532_SCALED');
      end
      ...
      if (~isempty(time) && ...
         ...
         bbp532 = gl_compute_BBP(time', ...
            cat(2, pres', temp', psal'), betaBackscattering', 532);

         eval(['o_rawData.' gl_get_path_to_data('') '.' bbpEgoVarName ' = bbp532'';']);
         g_decGl_directEgoVarPathName{end+1} = [gl_get_path_to_data('') '.' bbpEgoVarName];

         if (~isfield(g_decGl_derivedParamMetaData, bbpEgoVarName))
            g_decGl_derivedParamMetaData.(bbpEgoVarName) = gl_get_derived_param_meta_data('BBP532');
         end
      end
   end
end
...
for idP = 1:length(idBbp)
   ...
   if (~isempty(bbpEgoVarName) && isempty(bbpGliderVarName))
      ...
      % which BBP700 processing method is needed?
      if (isfield(g_decGl_calibInfo{g_decGl_calibInfoId}, 'ScaleFactBBP700') && ...
            isfield(g_decGl_calibInfo{g_decGl_calibInfoId}, 'DarkCountBBP700'))

         % BBP700 from BETA_BACKSCATTERING700
         betaBackscattering = get_values(o_rawData, 'BETA_BACKSCATTERING700');
      else
         % BBP700 from BETA_BACKSCATTERING700_SCALED
         betaBackscattering = get_values(o_rawData, 'BETA_BACKSCATTERING700_SCALED');
      end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% compute BBP880 derived parameter

idBbp = find(strncmp(g_decGl_egoVarName, 'BBP880', length('BBP880')));
for idP = 1:length(idBbp)
   bbpEgoVarName = g_decGl_egoVarName{idBbp(idP)};
   bbpGliderVarName = g_decGl_gliderVarName{idBbp(idP)};
   ...
   if (~isempty(bbpEgoVarName) && isempty(bbpGliderVarName))
      ...
      % which BBP880 processing method is needed?
      if (isfield(g_decGl_calibInfo{g_decGl_calibInfoId}, 'ScaleFactBBP880') && ...
            isfield(g_decGl_calibInfo{g_decGl_calibInfoId}, 'DarkCountBBP880'))

         % BBP880 from BETA_BACKSCATTERING880
         betaBackscattering = get_values(o_rawData, 'BETA_BACKSCATTERING880');
      else
         % BBP880 from BETA_BACKSCATTERING880_SCALED
         betaBackscattering = get_values(o_rawData, 'BETA_BACKSCATTERING880_SCALED');
      end
      ...
      if (~isempty(time) && ...
         ...
         bbp880 = gl_compute_BBP(time', ...
            cat(2, pres', temp', psal'), betaBackscattering', 880);

         eval(['o_rawData.' gl_get_path_to_data('') '.' bbpEgoVarName ' = bbp880'';']);
         g_decGl_directEgoVarPathName{end+1} = [gl_get_path_to_data('') '.' bbpEgoVarName];

         if (~isfield(g_decGl_derivedParamMetaData, bbpEgoVarName))
            g_decGl_derivedParamMetaData.(bbpEgoVarName) = gl_get_derived_param_meta_data('BBP880');
         end
      end
   end
end
...
for idP = 1:length(idCdom)
   ...
   if (~isempty(cdomEgoVarName) && isempty(cdomGliderVarName))
      ...
      if (~isempty(fluorescenceCdom))
         ...
         g_decGl_directEgoVarPathName{end+1} = [gl_get_path_to_data('') '.' cdomEgoVarName];
...
for idP = 1:length(idChla)
   ...
   if (~isempty(chlaEgoVarName) && isempty(chlaGliderVarName))
      ...
      if (~isempty(fluorescenceChla))
         ...
         g_decGl_directEgoVarPathName{end+1} = [gl_get_path_to_data('') '.' chlaEgoVarName];
...
for idP = 1:length(idNitrate)
   ...
   if (~isempty(nitrateEgoVarName) && isempty(nitrateGliderVarName))
      ...
      if (~isempty(time) && ...
         ...
         g_decGl_directEgoVarPathName{end+1} = [gl_get_path_to_data('') '.' nitrateEgoVarName];
...
for idP = 1:length(idTurbidity)
   ...
   if (~isempty(turbidityEgoVarName) && isempty(turbidityGliderVarName))
      ...
      if (~isempty(sideScatteringTurbidity))
         ...
         g_decGl_directEgoVarPathName{end+1} = [gl_get_path_to_data('') '.' turbidityEgoVarName];
...
if (~isempty(idF1))
   idF2 = find(strcmp(g_decGl_directEgoVarPathName, [gl_get_path_to_data('') '.' a_varName]), 1);
   if (~isempty(idF2) && gl_is_field_recursive(a_rawData, g_decGl_directEgoVarPathName{idF2}))
      % retrieve the data from g_decGl_directEgoVarPathName (case of a
      % 'slope_offset' derived parameter)
      eval(['o_values = a_rawData.' g_decGl_directEgoVarPathName{idF2} ';']);
