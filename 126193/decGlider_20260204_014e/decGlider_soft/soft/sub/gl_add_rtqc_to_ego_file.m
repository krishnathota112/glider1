...
%
% SYNTAX :
% [o_testDoneListAll, o_testFailedListAll] = gl_add_rtqc_to_ego_file( ...
%   a_ncEgoFilePathName, a_fromDecoderFlag)
%
% INPUT PARAMETERS :
%   a_ncEgoFilePathName : input EGO file path name
%   a_fromDecoderFlag   : 1 if the function is called from the decoder, 0
%                         otherwise
%
% OUTPUT PARAMETERS :
...
%                             specified).
%   12/12/2023 - RNU - V 1.10: Existing tests applied on BBP470_2, BBP700_2 and
%                              DOXY3 parameters.
%   01/27/2025 - RNU - V 1.11: Error in QC=4 report for parameters (except PRES)
%                              in TEST 19.
%   02/04/2025 - RNU - V 1.12: Added TEST 59: NITRATE specific test.
%                              Added TEST 63: CHLA specific test.
%                              Update of TEST 15: grey list test:
%                               - start/end dates can be provided in 'yyyymmdd'
%                               or 'yyyymmddHHMMSS' format
%                               - 'POSITION_QC' can be set through the grey list
%                              New general rule: "if PRES_QC=4 then <PARAM>_QC=4
%                              for parameters with at least one RTQC test"
%   03/10/2025 - RNU - V 1.13: TEST 20: added speed test on subsurface vs
%                              surface locations to flag inconsistent locations
%                              provided by Seaexplorer (ex: sea027_20241023 or
%                              sea003_20250205).
%   04/07/2025 - RNU - V 1.14: TEST 20: removed additionnal test set in V 1.13
%                              because it flagued to many SeaExplorer subsurface
%                              estimated positions (see sea017_20250227)
%                              Update the new general rule:
%                              "if PRES_QC=4 then <PARAM>_QC=4
%                              for parameters with at least one RTQC test"
%                              into
%                              "if PRES_QC=4 then <PARAM>_QC=4
%                              for parameters with at least one used RTQC test"
% ------------------------------------------------------------------------------
function [o_testDoneListAll, o_testFailedListAll] = gl_add_rtqc_to_ego_file( ...
   a_ncEgoFilePathName, a_fromDecoderFlag)
...
global g_decGl_paramWithRtqcTest;
...
testToPerformList = [ ...
   ...
   {'TEST059_NITRATE'} {g_decGl_rtqcTest59} ...
   {'TEST063_CHLA'} {g_decGl_rtqcTest63} ...
   ];
...
for idT = 1:2:length(testToPerformList)
   if (testToPerformList{idT+1} == 1)
      ...
      testFlagList(str2double(testName(5:7))) = 1;
   end
end
...
% index list of measurements with PRES_QC=4 and associated test number
presQC4MeasIdList = [];
presQC4TestNumList = [];
...
% DOXY3 (for comparison when multiple DO sensors are used)
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (testFlagList(3) == 1)
   ...
   if (~isempty(latitude) && ~isempty(longitude))
      ...
      idToCheck = find((latitude ~= paramLat.fillValue) & ...
         ...
         (positionQc ~= g_decGl_qcInterpolated)); % no need to select interpolated locations because their QC cannot be modified
...
if (testFlagList(4) == 1)
   ...
   if (~isempty(idToCheck))
      ...
      [uLonLat, ~, ic] = unique([longitudeGps(idToCheck)', latitudeGps(idToCheck)'], 'rows');

      % retrieve GEBCO elevations
      [elev] = gl_get_gebco_elev_point(uLonLat(:, 1), uLonLat(:, 2), gebcoPathFileName);
   ...
   if (~isempty(latitude) && ~isempty(longitude))
      ...
      if (~isempty(idToCheck))
         ...
         % ignore duplicated locations
         [uLonLat, ~, ic] = unique([longitude(idToCheck)', latitude(idToCheck)'], 'rows');
...
if (testFlagList(20) == 1)
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   % check LATITUDE_GPS and LONGITUDE_GPS using JAMSTEC QC
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   % check LATITUDE and LONGITUDE using a test on abnormal subsurface speeds
   
   % The following test has been disabled because it flags to many SeaExplorer
   % subsurface estimated positions (see sea017_20250227)
   % if (~isempty(latitude) && ~isempty(longitude))
   % 
   %    idToCheck = find( ...
   ...
   %       (timeQc ~= g_decGl_qcBad));
   % 
   %    idGoodGPS = find(positionGpsQc == g_decGl_qcGood);
   % 
   %    % initialize Qc flag
   %    positionQc(idToCheck) = gl_set_qc(positionQc(idToCheck), g_decGl_qcGood);
   %    testDoneList(20) = 1;
      ...
      if (rem(size(fileContents, 1), 7) ~= 0)
         ...
         if (~isempty(idJuldNoDef))
            ...
            for id = 1:length(idF)
               ...
               qcVal = int8(str2double(greyListInfo{idF(id), 5}));

               if (length(startDate) == 8)
                  startDateJuld = datenum(startDate, 'yyyymmdd') - g_decGl_janFirst1950InMatlab;
               elseif (length(startDate) == 14)
                  startDateJuld = datenum(startDate, 'yyyymmddHHMMSS') - g_decGl_janFirst1950InMatlab;
               end
               endDateJuld = '';
               if (~isempty(endDate))
                  if (length(endDate) == 8)
                  ...
                  elseif (length(startDate) == 14)
                     endDateJuld = datenum(endDate, 'yyyymmddHHMMSS') - g_decGl_janFirst1950InMatlab;
                  end
               end
               ...
               if (~isempty(idToFlag))
                  ...
                  if (~strcmp(param, 'POSITION_QC'))

                     % one PARAMETER to flag
                     for idD = 1:2
                        if (idD == 1)
                        ...
                        if (~isempty(idParam))
                           ...
                           if (~isempty(idNoDefParam))
                              ...
                              dataQc(idNoDefParam(idToFlagParam)) = gl_set_qc(dataQc(idNoDefParam(idToFlagParam)), qcVal);
                              eval([ncParamXDataQcList{idParam} ' = dataQc;']);
                              testFailedList(15) = 1;
                              o_testFailedListAll(15, idNoDefParam(idToFlagParam)) = 1;

                              if (strncmp(param, 'PRES', length('PRES')) && (qcVal == g_decGl_qcBad))
                                 presQC4MeasIdList = [presQC4MeasIdList, idNoDefParam(idToFlagParam)];
                                 presQC4TestNumList = [presQC4TestNumList, ones(size(idToFlagParam))*15];
                              end
                           end
                        end
                     end
                     ...
                     % POSITION_QC to flag
                     idNoDefQc = find(positionQc ~= g_decGl_qcDef);
                     if (~isempty(idNoDefQc))

                        % apply the test
                        idToFlagQc = find(ismember(idNoDefQc, idJuldNoDef(idToFlag)) == 1);
                        positionQc(idNoDefQc(idToFlagQc)) = gl_set_qc(positionQc(idNoDefQc(idToFlagQc)), qcVal);
                        testFailedList(15) = 1;
                        o_testFailedListAll(15, idNoDefQc(idToFlagQc)) = 1;
                     end
                  end
               end
            end
                  ...
                  for idP = 1:length(paramTestList)
                     ...
                     if (~isempty(idParam))
                        ...
                        if (~isempty(idNoDefParam))
                           ...
                           % apply the test
                           if (~isempty(idToFlag))
                              idToFlagParam = find(ismember(idNoDefParam, idNoDef(idToFlag)));
                              dataQc(idNoDefParam(idToFlagParam)) = gl_set_qc(dataQc(idNoDefParam(idToFlagParam)), g_decGl_qcBad);
                              eval([ncParamXDataQcList{idParam} ' = dataQc;']);
                              testFailedList(19) = 1;
                              o_testFailedListAll(19, idNoDefParam(idToFlagParam)) = 1;

                              if (strncmp(paramName, 'PRES', length('PRES')))
                                 presQC4MeasIdList = [presQC4MeasIdList, idNoDefParam(idToFlagParam)];
                                 presQC4TestNumList = [presQC4TestNumList, ones(size(idToFlagParam))*19];
                              end
                           end
                        end
                     end
...
if (testFlagList(6) == 1)
   ...
   paramTestMinMax = [ ...
      ...
      {-0.1} {50}; ... % CHLA2
      {-0.000025} {0.1}; ... % BBP700
      {-0.000025} {0.1}; ... % BBP700_2
      {-0.000005} {0.1}; ... % BBP470
      {-0.000005} {0.1}; ... % BBP470_2
      {-0.000005} {0.1}; ... % BBP532
               ...
               if (~strncmp(paramName, 'PRES', length('PRES')))
                  ...
                  for idT = 1:2
                     ...
                     if (~isempty(idPresToFlag))
                        ...
                        if (flagValue == g_decGl_qcBad)
                           presQC4MeasIdList = [presQC4MeasIdList, idPresNoDef(idPresToFlag)];
                           presQC4TestNumList = [presQC4TestNumList, ones(size(idPresToFlag))*6];
                        end
...
if (testFlagList(59) == 1)
   ...
   if (ismember('NITRATE', ncParamNameList))
      ...
      idParam = find(strcmp('NITRATE', ncParamXNameList) == 1, 1);
      data = eval(ncParamXDataList{idParam});
      dataQc = eval(ncParamXDataQcList{idParam});
      paramFillValue = ncParamXFillValueList{idParam};
      ...
      if (~isempty(idNoDef))
         ...
         testDoneList(59) = 1;
         o_testDoneListAll(59, idNoDef) = 1;

         testFailedList(59) = 1;
         o_testFailedListAll(59, idToFlag) = 1;
      end
   end
end
...
if (testFlagList(63) == 1)
   ...
   if (ismember('CHLA', ncParamNameList))
      ...
      idParam = find(strcmp('CHLA', ncParamXNameList) == 1, 1);
      data = eval(ncParamXDataList{idParam});
      dataQc = eval(ncParamXDataQcList{idParam});
      paramFillValue = ncParamXFillValueList{idParam};
      ...
      if (~isempty(idNoDef))
         ...
         testDoneList(63) = 1;
         o_testDoneListAll(63, idNoDef) = 1;

         testFailedList(63) = 1;
         o_testFailedListAll(63, idToFlag) = 1;
      end
   end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% General rule: 
% if PRES_QC=4 then <PARAM>_QC=4 for parameters with "at least one used RTQC test"

if (~isempty(presQC4MeasIdList))
   ...
   for idD = 1:2
      if (idD == 1)
         ...
         % retrieve PRESL data from the workspace
         presName = 'PRES_ADJUSTED';
         idPres = find(strcmp('PRES_ADJUSTED', ncParamXNameList) == 1, 1);
      end
      ...
      if (~isempty(idPres))
         ...
         if (~isempty(presData))
            ...
            if (any(presDataQc == g_decGl_qcBad))

               for idP = 1:length(g_decGl_paramWithRtqcTest)
                  paramName = g_decGl_paramWithRtqcTest{idP};
                  ...
                  if (~isempty(idParam))
                     ...
                     if (~isempty(paramData))
                        ...
                        % check that the RTQC tests have been used for this
                        % parameter
                        uParamDataQc = unique(paramDataQc);
                        uParamDataQc(uParamDataQc == g_decGl_qcDef) = [];
                        uParamDataQc(uParamDataQc == g_decGl_qcInterpolated) = [];
                        uParamDataQc(uParamDataQc == g_decGl_qcMissing) = [];

                        if (any(uParamDataQc ~= g_decGl_qcNoQc))
                           ...
                           idToFlag = find((presDataQc(idNoDef) == g_decGl_qcBad));
                           if (~isempty(idToFlag))
                              paramDataQc(idNoDef(idToFlag)) = gl_set_qc(paramDataQc(idNoDef(idToFlag)), g_decGl_qcBad);
                              eval([ncParamXDataQcList{idParam} ' = paramDataQc;']);

                              for idM = idNoDef(idToFlag)
                                 idF = find(presQC4MeasIdList == idM, 1, 'first');
                                 if (~isempty(idF))
                                    testFailedList(presQC4TestNumList(idF)) = 1;
                                    o_testFailedListAll(presQC4TestNumList(idF), idM) = 1;
                                 end
                              end
                           end
                        end
...
DIR_TMP_FILE = [ncEgoInputPath filesep 'tmp' filesep];
...
[ok] = nc_update_file(tmpNcEgoOutputPathFileName, ...
   dataQcList, testDoneHex, testFailedHex, a_fromDecoderFlag);
...
if (ok == 1)
   ...
   movefile(tmpNcEgoOutputPathFileName, [ncEgoOutputPath filesep fileName fileExtension]);
end
...
%
% SYNTAX :
% [o_ok] = nc_update_file(a_egoFileName, a_dataQc, ...
%   a_testDoneHex, a_testFailedHex, a_fromDecoderFlag)
%
% INPUT PARAMETERS :
...
%   a_fromDecoderFlag : 1 if the function is called from the decoder, 0 otherwise
%
% OUTPUT PARAMETERS :
...
%   04/11/2016 - RNU - V 1.0: creation
% ------------------------------------------------------------------------------
function [o_ok] = nc_update_file(a_egoFileName, a_dataQc, ...
   a_testDoneHex, a_testFailedHex, a_fromDecoderFlag)
