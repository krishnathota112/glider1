...
% add nc file data in jsonDeployData
if (g_decGl_seaGliderInputNc == 1)
   if (~isempty(a_dataDirPathName))
...
jsonDeployData = gl_check_json_deployment_file(jsonInputPathFile);
if (isempty(jsonDeployData))
   return
end
...
ok = write_json_deployment_file(jsonOutPathFile, egoJsonData, jsonDeployData, egoFileName);
if (ok == 0)
   return
end
...
tmpDirPathName = [jsonPathName filesep 'tmp' filesep];
...
try
   jsonDeployData = gl_load_json(a_deployJsonFilePathName);
catch MException
...
glSensorFile = jsonDeployData.glider_sensor;
for idS = 1:length(glSensorFile)
   if (isfield(glSensorFile(idS), 'sensor_file_name'))
      fileList{end+1} = [jsonPathName filesep glSensorFile(idS).sensor_file_name];
   end
end
...
%
% SYNTAX :
%  [o_ok] = write_json_deployment_file(a_jsonOutPathFile, a_egoJsonData, a_jsonDeployData)
%
% INPUT PARAMETERS :
%   a_jsonOutPathFile : file path name of the created json file
%   a_egoJsonData     : information for the EGO format
%   a_jsonDeployData  : information for the deployment
%
% OUTPUT PARAMETERS :
...
%   06/09/2013 - RNU - creation
% ------------------------------------------------------------------------------
function [o_ok] = write_json_deployment_file(a_jsonOutPathFile, a_egoJsonData, a_jsonDeployData, a_egoFileName)
...
for idF = 1:length(globAttList)
   if (any(strcmp(globAttList{idF}, egoFieldNames) == 1))
      ...
      data = a_jsonDeployData.global_attributes.(globAttList{idF});
   end
...
for id = 1:2:length(infoList)
   ...
   deployInfo = a_jsonDeployData.(inputName);
...
coordVar = a_jsonDeployData.coordinate_variables;
...
sensorData = a_jsonDeployData.glider_sensor;
...
% sort parameterList fields in the expected order
...
parameterListIn = parameterList;
parameterList = [];
for idP = 1:length(parameterListIn)
   paramStructIn = parameterListIn{idP};
   paramStruct = [];
   for idP2 = 1:length(paramExpInfoList)
      paramStruct.(paramExpInfoList{idP2}) = paramStructIn.(paramExpInfoList{idP2});
   end
   parameterList{end+1} = paramStruct;
end
...
egoInfoList = {
   ...
   'phase_management2', 'phase_management2', ...
   'positioning_method', 'positioning_method', ...
   'current_estimate', 'current_estimate', ...
   'surf_current_estimate', 'surf_current_estimate', ...
...
for idI = 1:2:length(egoInfoList)
   ...
   if (~isfield(a_egoJsonData, inputName))
      continue
   end
...
if ~(isfield(egoJsonData, 'EGO_format_version') && ...
      ...
      (str2double(egoJsonData.EGO_format_version) == g_decGl_egoFormatVersion))
   fprintf('ERROR: expected ''%.1f'' EGO format version in json file (%s)\n', ...
      g_decGl_egoFormatVersion, a_egoJsonFilePathName);
   return
end
if (isfield(egoJsonData, 'EGO_format_version'))
