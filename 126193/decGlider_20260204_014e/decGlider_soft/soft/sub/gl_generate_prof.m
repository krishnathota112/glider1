...
% minimum pressure to generate a profile file (in dbar)
global g_decGl_minProfPres;
   ...
   if (g_decGl_rtqcTest4 == 1)
      ...
      if ~(exist(g_decGl_rtqcGebcoFile, 'file') == 2)
         ...
         if (~isempty(idToCheck))
            ...
            [uLonLat, ~, ic] = unique([tabMeasPosLon(idToCheck), tabMeasPosLat(idToCheck)], 'rows');

            % retrieve GEBCO elevations
            [elev] = gl_get_gebco_elev_point(uLonLat(:, 1), uLonLat(:, 2), g_decGl_rtqcGebcoFile);
...
% remove shallow profiles
idPres = find(strcmp(tabParamName, 'PRES'));
if (~isempty(idPres))

   presVal = tabParamData(:, idPres);
   presFillVal = tabParamDef(idPres).fillValue;

   idDel = [];
   for idProf = 1:length(tabStart)
      idStart = tabStart(idProf);
      idStop = tabStop(idProf);
      profPres = presVal(idStart:idStop);
      profPres(profPres == presFillVal) = [];
      if (max(profPres) < g_decGl_minProfPres)
         idDel = [idDel idProf];
      end
   end
   tabStart(idDel) = [];
...
if (gl_att_is_present(fCdf, [], 'project_name'))
   projectName = strtrim(netcdf.getAtt(fCdf, netcdf.getConstant('NC_GLOBAL'), 'project_name'));
   if (~isempty(projectName))
      if (length(projectName) > 64)
         fprintf('WARNING: ''%s'' truncated to ''%s'' (original value ''%s'' = ''%s'') while generating NetCDF Argo profile from EGO file : %s\n', ...
            'PLATFORM_NUMBER', projectName(1:64), ...
            'wmo_platform_code', projectName, ...
            a_ncEgoFileName);
...
if (gl_att_is_present(fCdf, [], 'principal_investigator'))
   ...
   if (~isempty(piName))
      if (length(piName) > 64)
         fprintf('WARNING: ''%s'' truncated to ''%s'' (original value ''%s'' = ''%s'') while generating NetCDF Argo profile from EGO file : %s\n', ...
            'PLATFORM_NUMBER', piName(1:64), ...
            'wmo_platform_code', piName, ...
            a_ncEgoFileName);
         piName = piName(1:64);
      end
...
if (gl_att_is_present(fCdf, [], 'data_assembly_center'))
   dataCentre = strtrim(netcdf.getAtt(fCdf, netcdf.getConstant('NC_GLOBAL'), 'data_assembly_center'));
...
if (gl_var_is_present(fCdf, 'PLATFORM_TYPE'))
   platformType = strtrim(netcdf.getVar(fCdf, netcdf.inqVarID(fCdf, 'PLATFORM_TYPE'))');
   if (~isempty(platformType))
      if (length(platformType) > 32)
         fprintf('WARNING: ''%s'' truncated to ''%s'' (original value ''%s'' = ''%s'') while generating NetCDF Argo profile from EGO file : %s\n', ...
            'DATA_CENTRE', platformType(1:32), ...
            'DATA_CENTRE', platformType, ...
            a_ncEgoFileName);
         platformType = platformType(1:32);
      end
      tabMetaData = [tabMetaData {'PLATFORM_TYPE'} {platformType}];
   end
end
if (gl_var_is_present(fCdf, 'GLIDER_SERIAL_NO'))
   gliderSerialNo = strtrim(netcdf.getVar(fCdf, netcdf.inqVarID(fCdf, 'GLIDER_SERIAL_NO'))');
   if (~isempty(gliderSerialNo))
      if (length(gliderSerialNo) > 32)
         fprintf('WARNING: ''%s'' truncated to ''%s'' (original value ''%s'' = ''%s'') while generating NetCDF Argo profile from EGO file : %s\n', ...
            'DATA_CENTRE', gliderSerialNo(1:32), ...
            'DATA_CENTRE', gliderSerialNo, ...
            a_ncEgoFileName);
         gliderSerialNo = gliderSerialNo(1:32);
      end
      tabMetaData = [tabMetaData {'GLIDER_SERIAL_NO'} {gliderSerialNo}];
   end
end
if (gl_var_is_present(fCdf, 'FIRMWARE_VERSION_NAVIGATION'))
   ...
   if (~isempty(firmwareVersion))
      if (length(firmwareVersion) > 64)
         fprintf('WARNING: ''%s'' truncated to ''%s'' (original value ''%s'' = ''%s'') while generating NetCDF Argo profile from EGO file : %s\n', ...
            'FIRMWARE_VERSION', firmwareVersion(1:64), ...
            'FIRMWARE_VERSION_NAVIGATION', firmwareVersion, ...
            a_ncEgoFileName);
         firmwareVersion = firmwareVersion(1:64);
      end
...
tabProfiles = gl_add_mtime_in_profile(tabProfiles);

% generate the NetCDF Argo PROF files
o_generatedFiles = gl_create_nc_mono_prof_files_3_1(tabProfiles, tabMetaData, a_outputDirName, baseFileName);
