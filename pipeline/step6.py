...
def plot_ts_diagram(l1_path, plot_path=None):
    ...
    # Subsample for large datasets — stratified by depth to preserve deep water masses
    if n_valid > 50000:
        target = 50000
        depth_bands = [(0, 50), (50, 200), (200, 500), (500, 1100)]
        idx_all = []
        for d_lo, d_hi in depth_bands:
            band = np.where((D >= d_lo) & (D < d_hi))[0]
            if len(band) == 0:
                continue
            n_band = max(5000, int(target * len(band) / n_valid))
            n_band = min(n_band, len(band))
            idx_all.append(np.random.choice(band, n_band, replace=False))
        if idx_all:
            idx = np.concatenate(idx_all)
            if len(idx) > target:
                idx = np.random.choice(idx, target, replace=False)
        else:
            idx = np.random.choice(n_valid, target, replace=False)
        T, S, D = T[idx], S[idx], D[idx]

    fig, ax = plt.subplots(figsize=(9, 7))

    sc = ax.scatter(S, T, c=D, cmap="viridis_r",
                    s=4, alpha=0.7, edgecolor="none", rasterized=True)
                ...
                if np.sum(bad_mask) > 100:
                    ...
                    if bad_span_days > 5 and frac_bad > 0.05:
                        ...
                        if not already_flagged:
                            sensor_issues.append(
                                f"  ⚠  {'oxygen_concentration':22s}: "
                                f"{np.sum(bad_mask):,} Bad-flagged values "
                                f"({str(bad_times[0])[:10]} → "
                                f"{str(bad_times[-1])[:10]}, {len(bad_days)} days, "
                                f"{frac_bad*100:.0f}% of all O2 readings) "
                                f"— check for optode fouling/failure")
            ds1_tmp.close()
