...
parser.add_argument("--l0-source", default="auto",
                    choices=["auto", "pyglider", "decode"],
                    help="L0 source: 'pyglider' = use existing L0 from "
                         "L0-timeseries/ folder (skip decode), "
                         "'decode' = always decode from raw binaries, "
                         "'auto' = use pyglider L0 if found, else decode")
...
def main():
    ...
    # ── Resolve the L0 source ─────────────────────────────────────
    _explicit_l0 = getattr(_args, "l0_path", None)
    _reuse_l0    = _args.reuse_l0
    _l0_source   = _args.l0_source
    _skip1       = _args.skip_step1 or (_explicit_l0 is not None)

    # --l0-source takes precedence over legacy flags
    if _l0_source == "pyglider":
        # Always look for existing pyglider L0, never decode
        found_l0 = get_l0_path()
        if found_l0 and os.path.exists(found_l0):
            _skip1 = True
            _explicit_l0 = found_l0
            print(f"  --l0-source=pyglider: using {found_l0}")
        else:
            print(f"  --l0-source=pyglider: no L0 found in data dir!")
            print(f"  Looking for *.nc in L0-timeseries/ or data root...")
            # Search common locations
            import glob
            candidates = (
                glob.glob(os.path.join(_cfg.DATA_DIR, "L0-timeseries", "*.nc")) +
                glob.glob(os.path.join(_cfg.DATA_DIR, "*.nc"))
            )
            candidates = [c for c in candidates if "L1" not in os.path.basename(c)]
            if candidates:
                _explicit_l0 = max(candidates, key=os.path.getsize)
                _skip1 = True
                print(f"  Found: {_explicit_l0}")
            else:
                print(f"  ERROR: no L0 NetCDF found. Use --l0-source=decode "
                      f"to generate from raw binaries.")
                sys.exit(1)
    elif _l0_source == "decode":
        # Always decode from raw binaries
        _skip1 = False
        _explicit_l0 = None
        print(f"  --l0-source=decode: will decode from raw binaries")
    elif _l0_source == "auto":
        # Default: use pyglider L0 if found, else decode
        if not _skip1 and not _reuse_l0:
            found_l0 = get_l0_path()
            if found_l0 and os.path.exists(found_l0):
                _skip1 = True
                _explicit_l0 = found_l0
                print(f"  --l0-source=auto: found existing L0, using it: {found_l0}")
                print(f"  (use --l0-source=decode to force re-decode from binaries)")
            else:
                print(f"  --l0-source=auto: no existing L0, will decode from binaries")

    # Legacy --reuse-l0 flag (overridden by --l0-source if specified)
    if _reuse_l0 and not _skip1 and _l0_source == "auto":
        default = _l0_nc(dirs)
        found_l0 = default if os.path.exists(default) else get_l0_path()
        if found_l0 and os.path.exists(found_l0):
            _skip1 = True
            _explicit_l0 = found_l0
            print(f"  --reuse-l0: found existing L0: {found_l0}")
